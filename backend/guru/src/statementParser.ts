...
export async function parseStatement(pdfBuffer: Buffer): Promise<StatementParseResult> {
    try {
        ...
        if (result.parsedPurchases.length === 0) {
            result.error =
                'No promotional purchases found in the statement. This might be due to PDF formatting or text extraction issues.';
...
function parsePromotionalPurchasesByColumns(sectionText: string, fullText: string): ParsedPurchase[] {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (
            ...
            line.includes('outstanding balance') ||
            line.includes('promotion expiry')
        ) {
            headerLineIndex = i;
            break;
        }
...
function parsePurchaseLine(line: string, fullText: string): ParsedPurchase | null {
    try {
        ...
        console.log(
            '💰 Parsed amounts - Total:',
            components.totalAmount,
            'Outstanding:',
            components.outstandingBalance,
        );
        ...
        // Validate amounts
        if (
            components.totalAmount <= 0 ||
            components.outstandingBalance < 0 ||
            components.outstandingBalance > components.totalAmount
        ) {
            console.log(
                '❌ Invalid amounts - Total:',
                components.totalAmount,
                'Outstanding:',
                components.outstandingBalance,
            );
...
function parsePaymentRequirementsFromContext(
    currentLine: string,
    fullText: string,
): {
    ...
    if (contextText.match(/monthly\s*payments?\s*required/i)) {
        ...
        if (monthsMatch) {
            ...
            result.paymentType = 'monthly';
            console.log(
                '📅 Found monthly payment requirement from context for this purchase:',
                result.interestFreeMonths,
                'months',
            );
            return result;
        }
    ...
    if (interestFreeMatch) {
        ...
        if (contextText.match(/payment/i)) {
            ...
            result.paymentType = 'monthly';
            console.log(
                '📅 Found interest-free period with payment requirement for this purchase:',
                result.interestFreeMonths,
                'months',
            );
            return result;
            ...
            console.log(
                '📅 Found interest-free period but no payment requirement for this purchase:',
                interestFreeMatch[1],
                'months',
            );
        }
